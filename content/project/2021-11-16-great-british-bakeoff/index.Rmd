...
*Figure 1: The thirteen most commonly-used flavors in the the Great British Bake Off. Frequency of use is expressed as a total percentage of signature and showstopper bakes containing the flavor.*
...
Similarly, to address question #2, I calculated a "reward ratio". Values greater than 1 indicate that the change of getting star baker is *higher* if you use chocolate, whereas values less than 1 indicate that the chance of getting star baker is *lower*. Here's what I found for all the ingredients. 
...
```
*Figure 2. Relative risk and reward associated with using the 13 most-popular flavors in a round of the Great British Bake-off. For each flavor, a risk ratio greater than one means that a baker is more likely to be eliminated if they use the flavor compared to not using the flavor. A reward ratio greater than one means that a baker is more likely to win "star baker" if they use the flavor, compared to not using the flavor.*
...
*Figure 3. Animated illustration of the uncertainty in relative risk and reward associated with using the 13 most-popular flavors in a round of the Great British Bake-off. Each frame of the animation corresponds to the risk and reward ratios from a simulated data set generated by resampling the original data.*
...
*Figure 4. Static illustration of the uncertainty in relative risk and reward associated with using the 13 most-popular flavors in a round of the Great British Bake-off. Each point corresponds to the risk and reward ratios from a simulated data set generated by re-sampling the original data. Bars correspond to the risk and reward ratios calculated from the original dataset.*
...
In the end, there's still lots more to explore. Do certain judges prefer certain ingredients? How does a baker's performance in the technical challenge affect all of this?  For now, I'll have to leave those questions for another day.
